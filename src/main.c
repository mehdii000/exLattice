...
int main(void) {
  ...
  append_layer(network, 256); // Hidden layer
  append_layer(network, 128);
  append_layer(network, 64);
  ...
  for (int epoch = 0; epoch < MAX_EPOCHS; epoch++) {
    ...
    for (uint32_t i = 0; i < count; i += BATCH_SIZE) {
      uint32_t current_batch_size =
          (i + BATCH_SIZE > count) ? (count - i) : BATCH_SIZE;
      ...
      for (uint32_t b = 0; b < current_batch_size; b++) {
        ...
        nn_backward_pass(
            network, network->layers[network->layer_count - 1].neurons, target);
      }
      nn_update_weights(network, LEARNING_RATE, current_batch_size);

      if (i % 3200 == 0) {
        printf("Epoch %d, Progress: %.1f%%, Avg Loss: %f\r", epoch,
               (float)i / count * 100, total_loss / (i + current_batch_size));
        fflush(stdout);
      }
    }
    ...
    int correctly_predicted_images = 0;
    for (uint32_t i = 0; i < count; i++) {
      float *input = &all_images[i * MNIST_IMAGE_SIZE];
      nn_forward_pass(network, input);
      int predicted_label = 0;
      for (int j = 1; j < 10; j++) {
        if (network->layers[network->layer_count - 1].neurons[j] >
            network->layers[network->layer_count - 1].neurons[predicted_label])
          predicted_label = j;
      }
      if (predicted_label == all_labels[i])
        correctly_predicted_images++;
    }
    printf("Current Percentage Of Correctly Predicted Images: %.2f%%\n",
           (float)correctly_predicted_images / count * 100);
    fflush(stdout);
  }
